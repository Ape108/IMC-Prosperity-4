...
class VoucherStrategy(Strategy):
    ...
    def act(self, state: TradingState) -> None:
        ...
        # Construct IV smile and fit quadratic curve
        moneynesses, ivs = [], []
        ...
        if coeffs is None or self.strike / spot > self.max_otm_moneyness: return

        # Calculate theoretical price, Vega, and inventory ratio
        ...
        # --- THE FIX: Vega-Scaled Width & Aggressive Cubic Skew ---
        # 1. Calculate Vega. ATM options have the highest Vega, so they get the widest spreads naturally.
        option_vega = vega(spot, float(self.strike), tte_years, fitted_iv)
        
        # 2. Risk-adjusted dynamic width. 
        # Base of 1.5 + Vega adjustment (multiplier of 0.02) + Inventory expansion
        vega_width_expansion = option_vega * 0.02 
        dynamic_width = 1.5 + vega_width_expansion + (abs(inventory_ratio) * 3.0)
        
        # 3. Aggressive skew. Shifts theo by up to 5 ticks when nearing the limit to prevent pinning.
