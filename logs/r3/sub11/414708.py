...
class VoucherStrategy(Strategy):
    ...
    def act(self, state: TradingState) -> None:
        ...
        # Calculate theoretical price and inventory ratio
        fitted_iv = float(np.polyval(coeffs, self.strike / spot))
        theo_price = bs_call_price(spot, float(self.strike), tte_years, fitted_iv)
        position = state.position.get(self.symbol, 0)
        inventory_ratio = position / self.limit

        # --- THE FIX: Dynamic Width & Cubic Skew ---
        # Spread widens from 0.5 to 2.5 as inventory reaches limits
        dynamic_width = 0.5 + (abs(inventory_ratio) * 2.0)
        skew = (inventory_ratio ** 3) * 0.75
