...
class VoucherStrategy(Strategy):
    def __init__(self, symbol: str, limit: int, strike: int, max_otm_moneyness: float = 1.050) -> None:
        ...
        self.fitted_iv: float | None = None  # Injected by Trader
    ...
    def act(self, state: TradingState) -> None:
        if self.fitted_iv is None:
            return  # Safety skip if global curve fitting failed
        ...
        # Phase 2: Greek calculation & True Value (Using Fitted Global IV)
    ...
    def get_current_delta(self, state: TradingState) -> float:
        """Returns the total directional delta based on the fitted IV curve."""
        position = state.position.get(self.symbol, 0)
        ...
        # ACT ON IT: Uses the globally fitted curve instead of noisy isolated IVs
...
class Trader:
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        # ── PASS 0: Global IV Curve Fitting ─────────────
        ...
        if od and od.buy_orders and od.sell_orders:
            ...
            # Fit polynomial and distribute fitted IVs to options
