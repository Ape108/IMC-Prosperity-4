...
class VoucherStrategy(Strategy):
    ...
    def act(self, state: TradingState) -> None:
        ...
        option_delta = bs_call_delta(spot, float(self.strike), tte_years, fitted_iv)
        ...
        delta_penalty = 0.0
        if option_delta > 0.8:
            delta_penalty = (option_delta - 0.8) * 10.0 # Adds up to 2 extra ticks for delta ~1.0
            
        dynamic_width = 1.0 + vega_expansion + gamma_penalty + delta_penalty + (abs(inventory_ratio) * 3.0)
...
class HydrogelStrategy(MarketMakingStrategy):
    """
    Hydrogel Strategy using Microprice for true value, a 
    Linear Aggression Ramp for inventory management, and Dynamic Spreads 
    with Size Fading to protect against extreme tick-level volatility.
    """
    def get_true_value(self, state: TradingState) -> float:
    ...
    def act(self, state: TradingState) -> None:
        ...
        # Increased base aggression. Drops up to 5 ticks when full.
        aggression = 3.0 + (time_progress * 2.0) 
        skew = (inventory_ratio ** 3) * aggression
        skewed_true_value = true_value - skew
        
        # ── 2. DYNAMIC SPREAD WIDTH ───────────────────────────────────────────
        # Base spread of 1.0, widening up to 3.0 as inventory fills
        ...
        # ── 3. SIZE FADING (CLIP LIMITS) ──────────────────────────────────────
        # Never quote the full limit on a single tick. 
        # Cap at 40 so the skew has time to react to being filled during violent jumps.
        MAX_CLIP = 40
...
class HedgedVelvetfruitStrategy(MarketMakingStrategy):
    ...
    the aggregate delta of the options portfolio, protected by dynamic 
    widths and clip limits against directional jumps.
    """
    def get_true_value(self, state: TradingState) -> float:
        od = state.order_depths.get(self.symbol)
        if not od or not od.buy_orders or not od.sell_orders:
    ...
    def act_hedged(self, state: TradingState, aggregate_delta: float) -> None:
        ...
        # Inventory error is how far we are from our ideal hedge
        inventory_error = position - hedge_target
        inventory_error_ratio = inventory_error / self.limit
        
        # ── 1. AGGRESSIVE HEDGE SKEW ──────────────────────────────────────────
        # Increased skew base (2.0) to prioritize hitting the hedge target
        skew = (inventory_error_ratio ** 3) * 2.0 
        skewed_true_value = true_value - skew
        
        # ── 2. HEDGE-AWARE DYNAMIC WIDTH ──────────────────────────────────────
        # Base spread of 1.0, widens up to 3.5 ticks if we are severely unhedged
        ...
        # ── 3. SIZE FADING (CLIP LIMITS) ──────────────────────────────────────
        # Protects against 5-tick jumps. Max 40 per tick.
        MAX_CLIP = 40
        to_buy = min(self.limit - position, MAX_CLIP)
        to_sell = min(self.limit + position, MAX_CLIP)
