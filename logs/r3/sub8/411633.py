...
class MarketMakingStrategy(Strategy):
    ...
    def act(self, state: TradingState) -> None:
        ...
        skew_factor = 0.5  # Increase this to widen quotes more aggressively when holding inventory
...
class VoucherStrategy(Strategy):
    ...
    def __init__(self, symbol: str, 
                 limit: int, strike: int, k: float = 300.0, min_residual: float = 0.005, max_otm_moneyness: float = 1.020) -> None:
        super().__init__(symbol, limit)
        self.strike = strike
        self.k = k
