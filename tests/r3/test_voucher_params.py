...
def test_voucher_strategy_params():
    ...
    assert strat.max_otm_moneyness == 0.996
    assert strat.carry_window == 100
    assert strat.carry_threshold == 0.020
...
def test_all_vouchers_use_new_params():
    ...
    for strike in [4000, 4500, 5000, 5100, 5200, 5300, 5400, 5500, 6000, 6500]:
        ...
        assert strat.max_otm_moneyness == 0.996, f"VEV_{strike}: expected gate=0.996"
        assert strat.carry_window == 100, f"VEV_{strike}: expected carry_window=100"
        assert strat.carry_threshold == 0.020, f"VEV_{strike}: expected carry_threshold=0.020"
